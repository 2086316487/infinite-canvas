...
+ [新增] 远程图片生成改为异步任务模式，支持任务轮询、刷新恢复、失败返还和站内稳定图片 URL。
+ [新增] 新增图片任务 worker 启动角色和 Docker worker compose，用于拆分 Web/API 与图片任务消费进程。
+ [优化] 完善图片任务租约、有限重试、重启恢复和本地结果落盘流程，降低任务长期停留在运行中的风险。
+ [修复] 收紧生成图片公开媒体接口边界，仅允许访问已落盘的图片文件。
+ [修复] 固定文档站生产构建使用 webpack，避免 Next.js Turbopack 在 Docker 构建阶段异常中断。
+ [修复] 修复远程 API URL 工具重复定义，并调整部署文档代码块语言以适配文档站构建。
+ [修复] Docker 镜像工作流改为推送到当前仓库 owner 的 GHCR，支持 fork 仓库直接部署。
